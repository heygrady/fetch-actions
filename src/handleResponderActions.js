...
export const DEFAULT_HANDLER =
  '@@fetch-actions/handleResponderActions/DEFAULT_HANDLER'
...
export const handleResponderActions = (map) => (request, action) => {
  const type = selectActionType(action)
  invariant(
    type !== undefined,
    '@@fetch-actions/handleResponderActions action type must be defined. It is recommended that action be a valid flux-standard-action (https://github.com/acdlite/flux-standard-action)'
  )
  ...
  if (!handler) {
    warning(
      handler,
      `@@fetch-actions/handleResponderActions No handler matched action.type of ${type}. Using identityResponder which simply returns undefined.`
    )
