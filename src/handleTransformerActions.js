...
const handleTransformerActions = (map) => (json, action) => {
  ...
  invariant(
    type !== undefined,
    '@@fetch-actions/handleTransformerActions action type must be defined. It is recommended that action be a valid flux-standard-action (https://github.com/acdlite/flux-standard-action)'
  )
  ...
  if (!transformer) {
    warning(
      transformer,
      `@@fetch-actions/handleTransformerActions No transformer matched action.type of ${type}. Using identityHandler which simply returns json unchanged.`
    )
